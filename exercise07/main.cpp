...
bool intersects(Segment<T> s1, Segment<T> s2){
	...
	if( (sign(a) == 0 && sign(b) == 0) || (sign(c) == 0 && sign(d) == 0) ){
		return sign(dot_product(x3-x2,y3-y2,x3-x1,y3-y1)) <= 0 || sign(
				dot_product(x4-x2,y4-y2,x4-x1,y4-y1)) <= 0 
			|| sign(dot_product(x1-x3,y1-y3,x1-x4,y1-y4)) <= 0 || 
...
bool exist_intersections_handle_overflow(vector<vector<T>> & segments){
	...
	auto cmp = [&] (int i , int j) {
		...
		if(fA*deltaXB != fB*deltaXA) return fA*deltaXB < fB*deltaXA;
		return i < j;
	};
...
bool exist_intersections_bf(vector<vector<T>>& segments){
	if constexpr (is_same<T,int>::value){
		...
		for(int i = 0 ; i < sz(segments) ; i++){
			...
			for(int j = 0 ; j < 4 ; j++) seg[j] = segments[i][j];
			segs.push_back(seg);
		}
